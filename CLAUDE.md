...
- **On the fleet server, arm auto-merge — never park on CI.** A wake on this box is a *headless* run: when the turn ends the process exits and any background poller dies with it, so a PR left "waiting for CI" is a PR nobody is watching. Run `gh pr merge --auto --squash --delete-branch` in the **same turn** the PR is opened — GitHub merges it on green by itself, and the merge's own activity wakes the next run to verify `main` and post the report. Holding a window open to poll CI is an *attended* laptop behaviour only: the "parked on a near-term pollable signal" state in "Attended-Session Lifecycle Signal" presumes a human is watching the terminal, which on this box there is not.
