...
**Captain vs. capital.** The captain does **step 1 only**, and **step 1 is not finished until the handoff is posted** (step 1b). From the tag onward (steps 2–7) the **capital** owns the publish: it tags, the pipeline runs, the capital approves the `pypi` env-gate, then verifies the live install and closes the release issue. A release is done not at PyPI but when the live `pip install` is verified — and that whole tail belongs to the capital, not the founder.
